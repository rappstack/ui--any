...
import { dd_, div_, dl_, dt_, section_ } from 'relementjs/html'
...
export type dl_tree_basic_props_T = {
	...
	section_props?:Exclude<tag_props_T<HTMLElement>, 'class'>
	dt_class?:string
	...
	try {
		if ((<dl_tree_props_T>props____OR_basic_props_OR_dt_dd_pair__a1_).ctx?.is_ctx) {
			...
			if (_) {
				...
				section_class = class_(section_class, props2.section_class)
				section_props = { ...section_props, ...props2.section_props }
			...
			if (basic_props) {
				...
				section_class = class_(section_class, basic_props.section_class)
				section_props = { ...section_props, ...basic_props.section_props }
